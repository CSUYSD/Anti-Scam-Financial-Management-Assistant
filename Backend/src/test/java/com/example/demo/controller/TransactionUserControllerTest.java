...
public class TransactionUserControllerTest {
    ...
    public void testUpdateUser_Success() throws UserNotFoundException {
        // Given
        TransactionUser userDetails = new TransactionUser();
        userDetails.setUsername("updatedUser");

        doNothing().when(transactionUserService).updateUser(anyLong(), any(TransactionUser.class));

        // When
        ResponseEntity<String> response = transactionUserController.updateUser(1L, userDetails);

        // Then
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isEqualTo("User updated successfully");

        verify(transactionUserService, times(1)).updateUser(1L, userDetails);
    }
